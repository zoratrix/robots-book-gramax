---
order: 10
title: "Модуль Е: решение"
---

Нам предстоит поменять код пакета [turtlebro.delivery](http://turtlebro.delivery), а так же добавить его запуск в автозагрузку. Сначала делаем все изменения и проверяем, и только когда все полностью готово и работает, добавляем в автозагрузку и проверяем
...
-  delivery\_[deivices.py](http://deivices.py) - здесь описаны все ардуино устройства, подключенные к ROS робота

-  [delivery.py](http://delivery.py) - основной код, реализует логику доставки

-  delivery\_[speech.py](http://speech.py) - берет основной код [delivery.py](http://delivery.py) и добавляет в него озвучку

-  speech\_[client.py](http://client.py) - реализация озвучки (этот код нас не интересует)

Прежде чем менять код в файлах далее, скопируйте старые файлы (прямо в той же папке их можно оставить)

#### **1 Изменение delivery\_**[**devices.py**](http://devices.py)

Итак, у нас сейчас есть код вот такой:

delivery\_[devices.py](http://devices.py) (old)
...
создает издателя (publisher) под названием led_pub, который будет публиковать в топик /led_strip сообщения типа Int16 (сообщение с целым числом)
...
Файл delivery\_[devices.py](http://devices.py) готов

delivery\_[devices.py](http://devices.py) (new)
...
#### **2 Изменение** [**delivery.py**](http://delivery.py)
...
Прежде всего нам нужно в самом начале кожа импортировать класс LedStrip из delivery_devices. Найдите в начале строку:
...
И еще из delivery_devices нужно скопировать и вставить там же в начале [delivery.py](http://delivery.py) строчку:
...
Найдите вот этот кусок кода и рядом с [`self.top`](http://self.top)`_cap = TopCap()` и `self.start_button = Button()` добавьте `self.led_strip = LedStrip()`
...
Теперь, предположим, вы как минимум 5 раз проверили свой модифицированный пакет доставки, и готовы добавить его в автозагрузку. Немного информации об этом есть в инструкции к роботу: <https://manual.turtlebro.ru/paket-turtlebro/params#fail-turtlebro.launch>
